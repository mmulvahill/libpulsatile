...
spec <- pulse_spec(
  ...
  prior_sd_mass = 0.5, prior_sd_width = 5, prior_mass_mean = 2.5,
...
![plot of chunk trace](v04-convergence-diagnostics-trace-1.png)
...
round(ess_tbl, 1)
#>   baseline   halflife  mass_mean width_mean 
#>       82.3       49.0      220.4      133.9
```
...
round(gr, 3)
#>   baseline   halflife  mass_mean width_mean 
#>      1.004      1.007      1.003      1.000
```

Read together, these diagnostics tell a consistent story: most parameters reach
an adequate ESS with R-hat close to 1.0, while the half-life lags -- a larger
autocorrelation, a lower ESS, and an R-hat further from 1.0. That is a real and
common feature of this model, not a bug. Practical remedies are to run the chain
longer, thin more aggressively, or tighten the half-life prior when physiology
justifies it.
